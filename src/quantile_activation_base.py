...
def weighted_quantile_vectorized(values, quantiles, interpolation="linear"):
    ...
    # del values_context, positive_mask, negative_mask, sum_positives, sum_negatives, positive_weights, negative_weights, weights, sorted_indices, sorted_values, sorted_weights, cum_weights, total_weight, target_weights, above, below, value_below, value_above, weight_below, weight_above, interp_fraction
...
class CustomQuantFunction1D(torch.autograd.Function):
    ...
    def forward(ctx, inp, quant_list):
        ...
        with torch.no_grad():
            ...
            # # Removing unnecessary tensors from memory
            # del out_context, quantiles, densities, out
            # torch.cuda.empty_cache()
            # gc.collect()
...
class quantile_activation_1d(nn.Module):
    def __init__(self, n_features):
        super().__init__()
        self.bn_in = nn.BatchNorm1d(n_features, affine=True, track_running_stats=False)
        self.bn_out = nn.BatchNorm1d(n_features, affine=True, track_running_stats=False)
        self.quantile_list = nn.Parameter(torch.linspace(0.01, 0.99, 102), requires_grad=False)

    def forward(self, x):
        out = self.bn_in(x)
        out = CustomQuantFunction1D.apply(out, self.quantile_list)
        out = self.bn_out(out)
        return out
...
class CustomQuantFunction2D(torch.autograd.Function):
    def forward(ctx, inp, quant_list):
        ...
        with torch.no_grad():
            ...
            out_context = inp.clone().transpose(0, 1).flatten(1)
            quantiles = weighted_quantile_vectorized(out_context, quant_list)
            densities = weighted_vectorized_kde(out_context, quantiles, 0.5)
            out = quant_list[
                torch.searchsorted(quantiles, out_context, side="right").clamp_(
            ...
            # # Removing unnecessary tensors from memory
            # del out_context, quantiles, densities, out,shape
            # torch.cuda.empty_cache()
            # gc.collect()
...
class quantile_activation_2d(nn.Module):
    ...
    def forward(self, x):
        # Batch normalization
        out = self.bn(x)
        out = CustomQuantFunction2D.apply(out, self.quant_list)
        out = self.bn_out(out)
        return out
